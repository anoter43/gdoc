...
This is a python rewrite of [scdoc](git.sr.ht/~sircmpwn/scdoc)
...
Each gdoc file must begin with the following preamble:

	*name*(_section_) ["left\_footer" ["center\_header"]]

**name** is the name of the man page you are writing, and <ins>section</ins> is the section
...
Text can be made **bold** or <ins>underlined</ins> with asterisks and underscores: \*bold\*
...
&nbsp;&nbsp;&nbsp;&nbsp;You may use multiple lines and most <ins>formatting</ins>.
...
- Item 1 is pretty long so let's<br/>
  break it up onto two lines
- Item 2 is shorter
	- But its children can go on<br/>
	  for a while
...
1. Item 1
2. Item 2
3. Item 3,<br/>with multiple lines
...
align), = (center align), and > (right align), like so (Github markdown can't
render it properly, I tried, but gave up.):
...
You may turn off gdoc formatting and output literal text with escape codes and
...
````
```
_This formatting_ will *not* be interpreted by gdoc.
```
````
